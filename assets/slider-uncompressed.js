...
var Cfslider = new Class({
    ...
    checkStep: function(mx){
        ...
        if(this.maxknob!=null) {
            ...
            this.clip_l = parseInt(this.knob.getStyle('left')) + parseInt(parseInt(this.options.knobwidth)/2);
            var w = Math.abs(parseInt(this.knob.getStyle('left')) - parseInt(this.maxknob.getStyle('left')));
            var width = parseInt(this.clip_l + w);
            this.bkg.style.clip = "rect(0px "+  width + "px 10px "+ this.clip_l + "px)" ;

        }else {
            this.fireEvent('onChange', this.step);
            this.bkg.style.clip = "rect(0px "+  (parseInt(this.drag.value.now[this.z]))  + "px 10px 0px)";
        }
    },
